...
server.use((req, res, next) => {
  res.header("Access-Control-Allow-Origin", "https://talkap.com.ar"); // update to match the domain you will make the request from
...
server.post("/donador", async (req, res) => {
  ...
  const confirmacion = listoparadonar
    ? usuariodespuesdedonar === listoparadonar
    : false; ///bueno basicamente compara el estado antes de que el usuario done y despues de que dona (para evitar que el mensaje de gracias aparezca siempre)
  const mensajedeconfirmacion = confirmacion
    ? "gracias por donar"
    : "usuario sin donaciones o sin donaciones recientes";

  if (ultimodonador == user && closestObject?.email.includes(user)) {
    ///comparamos de tal manera que responda al donador
